...
pub fn main() anyerror!void {
    ...
    while (node_iterator.next()) |node| {
        if (node.value.prereqs_remaining == 0) {
            insert_by_ascii_value(node.key, &current_execution);
        }
    }

    while (current_execution.len > 0) {
        var step = current_execution.pop();
        _ = results.append(step);

        var node = nodes.get(step).?;
        var children_iterator = node.value.children.iterator();
        while (children_iterator.next()) |child_step| {
            var child_node = nodes.get(child_step).?;
            child_node.value.prereqs_remaining -= 1;

            if (child_node.value.prereqs_remaining == 0) {
                insert_by_ascii_value(child_step, &current_execution);
            }
        }
