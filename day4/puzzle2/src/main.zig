...
fn parseLines(lines: []const []const u8, results: *std.AutoHashMap(u64, [60]u64)) void {
    var start_sleep: u8 = 0;
    var guard_id: u64 = 0;

    for (lines) |line| {
        var elements = std.mem.split(line, " ");
        _ = elements.next(); // discard date
        var time = elements.next();
        var minutes = std.fmt.parseInt(u8, time.?[3..5], 10) catch unreachable;
        var event = elements.next();
        switch (event.?[0]) {
            'G' => {
                var guard_id_string = elements.next();
                guard_id = std.fmt.parseInt(u64, guard_id_string.?[1..], 10) catch unreachable;
                var result = results.getOrPut(guard_id) catch unreachable;
                if (!result.found_existing) {
                    result.kv.value = []u64{0} ** 60;
                }
            },
            'w' => {
                var entry = results.get(guard_id);
                var i = start_sleep;
                while (i < minutes) : (i +=1) {
                    entry.?.value[i] += 1;
                }
...
pub fn main() anyerror!void {
    ...
    // we can lexically sort the lines
    var split_lines = std.ArrayList([]const u8).init(&direct_allocator.allocator);
    var lines = std.mem.split(input, "\n");

    while (lines.next()) |line| {
        _ = split_lines.append(line);
    }

    std.sort.sort([]const u8, split_lines.toSlice(), lessThan);

    // id to histogram of sleeping mins
    var histograms = std.AutoHashMap(u64, [60]u64).init(&direct_allocator.allocator);

    parseLines(split_lines.toSlice(), &histograms);

    var guards = histograms.iterator();
    var best_id: u64 = 0;
    var best_time: u64 = 0;
    var best_minute: usize = 0;
    while (guards.next()) |guard| {
        var id = guard.key;
        var longest_minute: usize = 0;
        var longest_time: u64 = 0;
        for (guard.value) |time, i| {
        ...
        if (longest_time > best_time) {
            best_id = id;
            best_time = longest_time;
            best_minute = longest_minute;
        }
    }

    std.debug.warn("id {} minute {} time {}\n", best_id, best_minute, best_time);
